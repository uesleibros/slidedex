...
class TradeRequestView(discord.ui.View):
	def __init__(self, initiator: discord.Member, target: discord.Member, timeout: float = 60.0):
	...
	async def accept_button(self, interaction: discord.Interaction, button: discord.ui.Button):
		if interaction.user.id != self.target.id:
			return await interaction.response.send_message(
				"Apenas a pessoa convidada pode aceitar!",
				ephemeral=True
			)
		...
		embed = discord.Embed(
			...
			description=f"{self.target.mention} aceitou a solicitação de trade!",
	...
	async def decline_button(self, interaction: discord.Interaction, button: discord.ui.Button):
		if interaction.user.id != self.target.id:
			return await interaction.response.send_message(
				"Apenas a pessoa convidada pode recusar!",
				ephemeral=True
			)
		
		embed = discord.Embed(
			title="Trade Recusada",
			description=f"{self.target.mention} recusou a solicitação de trade.",
	...
	async def on_timeout(self):
		if self.message:
			embed = discord.Embed(
				...
				description=f"{self.target.mention} não respondeu a tempo.",
				color=discord.Color.orange()
			)
			await self.message.edit(embed=embed, view=None)
			await self.message.channel.send(f"{self.initiator.mention}, sua solicitação de trade expirou.")
