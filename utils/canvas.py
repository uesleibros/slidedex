...
def _compose_pokemon(
	...
	try:
		...
		spr = _process_sprite_crop(sprite_bytes, sprite_box_size, sprite_box_size, crop=False)
...
def _compose_battle(
	...
	try:
		if player_bytes:
			...
			p = _process_sprite_crop(player_bytes, player_box_size, player_box_size, crop=False, scale_boost=0.7, max_height_ratio=0.95)
			composed.paste(p, (player_x, player_ground_y - p.height), p)
		if enemy_bytes:
			enemy_box_size = int(box_size * 0.6)
			e = _process_sprite_crop(enemy_bytes, box_size, box_size, crop=False, scale_boost=1.0, max_height_ratio=0.75)
