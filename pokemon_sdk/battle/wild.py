...
def _build_effect_examples(mapper: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    examples: Dict[str, Dict[str, Any]] = {}
    for desc, val in mapper.items():
        effs = val.get("effects", []) or []
        for e in effs:
            et = e.get("type")
            if not et:
                continue
...
class MoveData:
    def __init__(
        self, name: str, accuracy: Optional[int], power: int, priority: int,
        dmg_class: str, type_name: str, min_hits: int, max_hits: int, flinch: int,
        drain: int, recoil: int, healing: int, ailment: Optional[str],
        ailment_chance: int, stat_changes: List[Tuple[str,int,bool]]
    ):
        self.name = name
...
def _canon_stat(s: str) -> Optional[str]:
    return {"attack": "atk", "defense": "def", "special-attack": "sp_atk",
            "special-defense": "sp_def", "speed": "speed"}.get(s)
...
def _normalize_move(move) -> MoveData:
    ...
    try:
        ...
        pass
    return MoveData(
        name=name, accuracy=accuracy, power=power, priority=priority,
        dmg_class=dmg_class, type_name=type_name, min_hits=min_hits,
        max_hits=max_hits, flinch=flinch, drain=drain, recoil=recoil,
        healing=healing, ailment=ailment, ailment_chance=ailment_chance,
