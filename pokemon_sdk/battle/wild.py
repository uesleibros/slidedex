...
def _stage_mult(stage: int) -> float: return (2+stage)/2 if stage>=0 else 2/(2-stage)

def _apply_stage(base: int, stage: int) -> int: return max(1, int(base * _stage_mult(stage)))

def _types_of(p: "BattlePokemon") -> List[str]: try: return [t.type.name.lower() for t in p.pokeapi_data.types] except: return []

def _type_mult(atk_type: str, def_types: List[str]) -> float: atk = (atk_type or "").lower() if atk not in TYPE_CHART: return 1.0 m = 1.0 for d in def_types: if d in TYPE_CHART[atk]["immune"]: return 0.0 if d in TYPE_CHART[atk]["super"]: m *= 2.0 elif d in TYPE_CHART[atk]["not"]: m *= 0.5 return m

def _hp_bar(c: int, t: int, l: int=10) -> str: p = 0 if t<=0 else max(0.0, min(1.0, c/t)) f = int(round(l*p)) bar = "█"f + "░"(l-f) return f"[{bar}]"

def _slug(move_id: Any) -> str: if move_id is None: return "" s = str(move_id).strip().lower() return s.replace(" ", "-")

def _build_effect_examples(mapper: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]: examples: Dict[str, Dict[str, Any]] = {} for desc, val in mapper.items(): effs = val.get("effects", []) or [] for e in effs: et = e.get("type") if not et: continue if et not in examples: examples[et] = e.copy() return examples

_effect_examples = _build_effect_examples(effect_mapper)

class MoveData: def init(self, name: str, accuracy: Optional[int], power: int, priority: int, dmg_class: str, type_name: str, min_hits: int, max_hits: int, flinch: int, drain: int, recoil: int, healing: int, ailment: Optional[str], ailment_chance: int, stat_changes: List[Tuple[str,int,bool]] ): self.name=name; self.accuracy=accuracy; self.power=power; self.priority=priority self.dmg_class=dmg_class; self.type_name=type_name self.min_hits=min_hits; self.max_hits=max_hits self.flinch=flinch; self.drain=drain; self.recoil=recoil; self.healing=healing self.ailment=ailment; self.ailment_chance=ailment_chance self.stat_changes=stat_changes

def _canon_stat(s: str) -> Optional[str]: return {"attack":"atk","defense":"def","special-attack":"sp_atk","special-defense":"sp_def","speed":"speed"}.get(s)

def _pick_frlg(move) -> Dict[str, Optional[int]]: try: for pv in getattr(move, "past_values", []) or []: vg = getattr(pv.version_group, "name", "") if vg == "firered-leafgreen": return { "accuracy": pv.accuracy if pv.accuracy is not None else move.accuracy, "power": pv.power if pv.power is not None else move.power, "pp": pv.pp if pv.pp is not None else getattr(move, "pp", None), "effect_chance": pv.effect_chance if pv.effect_chance is not None else getattr(move, "effect_chance", None) } except: pass return {"accuracy": move.accuracy, "power": move.power, "pp": getattr(move, "pp", None), "effect_chance": getattr(move, "effect_chance", None)}

def _normalize_move(move) -> MoveData: name = getattr(move, "name", "move").replace("-"," ").title() type_name = getattr(getattr(move, "type", None), "name", "normal") dc = getattr(getattr(move, "damage_class", None), "name", None) dmg_class = dc if dc in {"physical","special","status"} else ("physical" if (getattr(move,"power",0) or 0) > 0 else "status") pv = _pick_frlg(move) accuracy = pv["accuracy"] power = int(pv["power"] or 0) priority = int(getattr(move, "priority", 0) or 0) meta = getattr(move, "meta", None) min_hits = int(getattr(meta, "min_hits", 1) or 1) max_hits = int(getattr(meta, "max_hits", 1) or 1) flinch = int(getattr(meta, "flinch_chance", 0) or 0) drain = int(getattr(meta, "drain", 0) or 0) recoil = int(getattr(meta, "recoil", 0) or 0) healing = int(getattr(meta, "healing", 0) or 0) ail = getattr(getattr(meta, "ailment", None), "name", "none") ailment = None if ail in {None,"none","unknown"} else ail ailment_chance = int(getattr(meta, "ailment_chance", 0) or pv["effect_chance"] or (100 if (dmg_class=="status" and ailment) else 0) or 0) target_self_default = getattr(getattr(move, "target", None), "name", "") in {"user","user-or-ally","ally"} stat_changes = [] try: for sc in getattr(move, "stat_changes", []) or []: raw = getattr(getattr(sc, "stat", None), "name", None) delta = int(getattr(sc, "change", 0) or 0) canon = _canon_stat(raw) if raw else None if canon and delta != 0: target_self = target_self_default if delta>0 else not target_self_default stat_changes.append((canon, delta, target_self)) except: pass return MoveData( name=name, accuracy=accuracy, power=power, priority=priority, dmg_class=dmg_class, type_name=type_name, min_hits=min_hits, max_hits=max_hits, flinch=flinch, drain=drain, recoil=recoil, healing=healing, ailment=ailment, ailment_chance=ailment_chance, stat_changes=stat_changes )

class BattlePokemon: def init(self, raw: Dict[str, Any], pokeapi_data: aiopoke.Pokemon, species_data: aiopoke.PokemonSpecies): self.raw = raw self.species_id = raw["species_id"] self.name = raw.get("name") self.level = raw["level"] base_stats = pm.service.get_base_stats(pokeapi_data) self.stats = calculate_stats(base_stats, raw["ivs"], raw["evs"], raw["level"], raw["nature"]) self.current_hp = raw.get("current_hp") or self.stats["hp"] self.moves = raw.get("moves") or [{"id":"tackle","pp":35,"pp_max":35}] self.pokeapi_data = pokeapi_data self.species_data = species_data self.is_shiny = raw.get("is_shiny", False) self.stages = {"atk":0,"def":0,"sp_atk":0,"sp_def":0,"speed":0} self.status = {"name": None, "counter": 0} self.volatile = {"flinch": False, "confuse": 0} self.sprites = { "front": pokeapi_data.sprites.front_shiny if self.is_shiny else pokeapi_data.sprites.front_default, "back":  pokeapi_data.sprites.back_shiny  if self.is_shiny else pokeapi_data.sprites.back_default }
...
class WildBattle: def init(self, player_party: List[Dict[str, Any]], wild: Dict[str, Any], user_id: str, interaction: discord.Interaction) -> None: self.user_id = user_id self.interaction = interaction self.player_party_raw = player_party self.active_player_idx = 0 self.wild_raw = wild self.ended = False self.turn = 1 self.message: Optional[discord.Message] = None self.lock = asyncio.Lock() self.player_team: List[BattlePokemon] = [] self.wild: Optional[BattlePokemon] = None self.actions_view: Optional[WildBattleView] = None self.lines: List[str] = [] self.must_redraw_image = True self.move_cache: Dict[str, MoveData] = {}
...
def _apply_secondary(self, user: BattlePokemon, target: BattlePokemon, md: MoveData, total: int) -> List[str]:
	...
	if md.recoil != 0 and total > 0:
		amt = max(1, int(total * abs(md.recoil) / 100))
		user.current_hp = max(0, user.current_hp - amt)
		out.append(f"{user.name.title()} sofreu {amt} de recuo!")
	...
	if md.ailment:
		...
		if random.randint(1,100) <= ch:
			...
			if applied:
				...
				out.append(f"O alvo {texts.get(md.ailment, md.ailment+'u')}!")
	return out
...
class MovesView(discord.ui.View): def init(self, battle: WildBattle, timeout: float = 60.0): super().init(timeout=timeout) self.battle = battle for mv in battle.player_active.moves: key = _slug(mv["id"]) md = battle.move_cache.get(key) label_text = (md.name if md else key.replace("-"," ").title()) pp, pp_max = mv["pp"], mv["pp_max"] btn = discord.ui.Button(style=discord.ButtonStyle.primary, label=f"{label_text} ({pp}/{pp_max})", disabled=(pp <= 0)) btn.callback = self._cb(mv["id"]) self.add_item(btn) back = discord.ui.Button(style=discord.ButtonStyle.secondary, label="Voltar") async def back_cb(i: discord.Interaction): if str(i.user.id)!=battle.user_id: return await i.response.send_message("Não é sua batalha!", ephemeral=True) await i.response.edit_message(view=battle.actions_view) back.callback = back_cb self.add_item(back)
...
class SwitchView(discord.ui.View): def init(self, battle: WildBattle, force_only: bool = False, timeout: float = 60.0): super().init(timeout=timeout) self.battle = battle for i, p in enumerate(battle.player_team): lbl = f"{i+1}. {p.name.title()} ({max(0,p.current_hp)}/{p.stats['hp']})" btn = discord.ui.Button(style=discord.ButtonStyle.success, label=lbl, disabled=p.fainted or i==battle.active_player_idx) btn.callback = self._mk(i); self.add_item(btn) if not force_only: back = discord.ui.Button(style=discord.ButtonStyle.secondary, label="Voltar") async def back_cb(i: discord.Interaction): if str(i.user.id)!=battle.user_id: return await i.response.send_message("Não é sua batalha!", ephemeral=True) await i.response.edit_message(view=battle.actions_view) back.callback = back_cb self.add_item(back)
...
class WildBattleView(discord.ui.View): def init(self, battle: WildBattle, timeout=60.0): super().init(timeout=timeout) self.battle = battle self.user_id = battle.user_id self.force_switch_mode = False
