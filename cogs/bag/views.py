...
class PokemonInfoLayout(discord.ui.LayoutView):
    ...
    def _build_static_text(self):
        ...
        self._current_moves_text = [
            f"**{move['id'].replace('-', ' ').title()}** ({move['pp']}/{move['pp_max']} PP)" 
            for move in pokemon.get("moves", [])
        ]
        
        self._future_moves_text = [
            f"**Lv. {lvl}:** {name.replace('-', ' ').title()}" 
            for lvl, name in self._future_moves[:8]
        ]

    def _get_iv_stats_lines(self) -> Tuple[List[str], str]:
        pokemon = self.current_pokemon
        ...
        return stats_lines, "attachment://iv.png"

    def _get_ev_stats_lines(self) -> Tuple[List[str], str]:
        pokemon = self.current_pokemon
        stats = self._cached_stats
        current_hp = pokemon.get("current_hp") if pokemon.get("current_hp") is not None else stats["hp"]
        base_stats = pokemon['base_stats']
        ...
        return stats_lines, "attachment://ev.png"
    ...
    def build_page(self):
        ...
        stats_section.add_item(discord.ui.TextDisplay(self._stats_info_text))
        container.add_item(stats_section)
        container.add_item(discord.ui.Separator())

        if self.show_iv:
            stats_lines, thumbnail = self._get_iv_stats_lines()
            self._toggle_stats_btn.label = "Mostrar EVs"
        else:
            stats_lines, thumbnail = self._get_ev_stats_lines()
