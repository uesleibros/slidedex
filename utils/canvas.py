...
def _compose_pokemon(
	...
	try:
		sprite_box_size = int(box_size)
		spr = _process_sprite_crop(sprite_bytes, sprite_box_size, sprite_box_size, crop=False, scale_boost=0.5, max_height_ratio=0.5, force_height=True)
...
def _compose_battle(
	...
	try:
		if player_bytes:
			player_box_size = int(box_size)
			p = _process_sprite_crop(player_bytes, player_box_size, player_box_size, crop=False, scale_boost=0.85, force_height=True, max_height_ratio=0.95)
			composed.paste(p, (player_x, player_ground_y - p.height), p)
		if enemy_bytes:
			enemy_box_size = int(box_size)
			e = _process_sprite_crop(enemy_bytes, box_size, box_size, crop=False, scale_boost=0.6, max_height_ratio=0.6, force_height=True)
